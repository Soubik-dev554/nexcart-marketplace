  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1000px', margin: '0 auto' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '15px', marginBottom: '5px' }}>
        <img src="/NEXCARTLogo.png" alt="Logo" style={{ height: '40px', width: '40px', borderRadius: '8px', objectFit: 'cover', filter: 'drop-shadow(0 0px 10px rgba(249, 115, 22, 0.3))' }} />
      ...
      {stats ? (
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px' }}>
          ...
          <div style={cardStyle}>
            <h4 style={{ color: '#a1a1aa', fontSize: '1rem' }}>Total Customers</h4>
      ...
      <div style={{ marginTop: '40px', padding: '30px', background: '#18181b', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.05)' }}>
        <h3 style={{ marginBottom: '25px', color: '#f97316' }}>Seller Controls</h3>
        <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
          <button className="btn" onClick={() => navigate('/seller/add-product')}>+ Add Product</button>
          <button className="btn" onClick={() => navigate('/seller/products')} style={{ background: '#3f3f46' }}>📦 My Products</button>
          <button className="btn" onClick={() => navigate('/seller/orders')} style={{ background: '#3f3f46' }}>🚚 Customer Orders</button>
          <button className="btn" onClick={() => navigate('/seller/users')} style={{ background: '#3f3f46' }}>👥 Customers</button>
        </div>
      </div>
    </div>
  );
